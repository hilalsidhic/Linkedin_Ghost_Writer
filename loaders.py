...
def load_mhtml(filepath: str):
    """Load MHTML file and return documents"""
    loader = MHTMLLoader(filepath)
    return loader.load()

def extract_posts(docs):
    """Extract LinkedIn posts from loaded documents"""
    posts = []
    ...
    for doc in docs:
        ...
        for chunk in chunks[1:]:
            ...
            impressions, likes, comments = get_impressions(extracted)
...
def get_impressions(extracted: str) -> tuple[int, int, int]:
    """Extract impressions, likes, and comments from post text"""
    impressions = 0
